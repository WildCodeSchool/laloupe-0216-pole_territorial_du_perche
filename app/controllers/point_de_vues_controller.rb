class PointDeVuesController < ApplicationController
  ...
  def create
    ...
    if point_de_vue.save
      ...
      redirect_to point_de_vues_path, method: :get, notice: "Votre point de vue a été pris en compte. Il sera visible dans les plus bref délais. Nous vous invitons à déposer d’autres points de vue #{point_de_vue.positif ? 'négatif' : 'positif'}."
    else
      render 'new'
    end
  end
